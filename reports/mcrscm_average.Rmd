...
params:
  cache.path:
    ...
    value: "/home/danielred/data/programs/mcrs_to_scm/OUT/A7long.1_6/temp/"
  dir:
    label: Path to data dir
    value: "/home/danielred/data/programs/mcrs_to_scm/OUT/A7long.1_6/"
  ssh:
...
```{r load_funcs, message=FALSE, cache=TRUE}
source("../functions.R")
source("../xml.R")
...
```{r plot_everything, eval=output_refreshed, fig.show='hide', eval=F}
if(nrow(table_long) > 100000){
  warning("Too much data, quitting!")
  knitr::knit_exit()
}
...
```{r check_savefiles}
if(file.exists("opened.Rds")) {
  opened <- readRDS("opened.Rds")
} else {
  opened <- character()
}
...
```{r load_savefiles, eval=newsave}
if(file.exists("opened.Rds") & !params$force) {
  savedata <- readRDS("savedata.Rds")
} else {
  savedata <- list()
  savedata$mosaic <- data.frame()
}
...
for(savefile in toread){
  ...
  # mosaic data
  try({
    comp = getLastComposition(savefile, exclude = T)
...
# write opned.Rds
saveRDS(opened, "opened.Rds")

# write savedata.Rds
saveRDS(savedata, "savedata.Rds")
```
